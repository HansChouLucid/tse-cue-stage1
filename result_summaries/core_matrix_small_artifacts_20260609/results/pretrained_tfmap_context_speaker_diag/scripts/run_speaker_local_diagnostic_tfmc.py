...
def main():
    ...
    chunk_f.close()
    utt=pd.DataFrame(utt_rows)
    sis_col = 'sisnr_i' if 'sisnr_i' in inf.columns else ('si_snri' if 'si_snri' in inf.columns else None)
    if sis_col is not None:
        utt=utt.merge(inf[['key',sis_col]].rename(columns={sis_col:'sisnr_i'}),on='key',how='left')
    else:
        utt['sisnr_i']=float('nan')
